...
export function setSign(config: AxiosRequestConfig, timestamp: Number): string {
  ...
  return encryptByMd5(str + encryptByMd5(timestamp.toString().substring(0, 8)) + timestamp)
}
...
export function setUploadSign() {
  ...
  return {
    timestamp,
    version,
    sign: encryptByMd5('' + encryptByMd5(timestamp.toString().substring(0, 8)) + timestamp)
  }
}
