import { createGlobalState, RemovableRef, useStorage, useSessionStorage, StorageSerializers } from '@vueuse/core'
...
export const useGlobalState = createGlobalState((): GlobalState => {
  // 语言
  const language: RemovableRef<string> = useStorage('vueuse-local-language', config.language)

  // 令牌
  const token: RemovableRef<string> = useStorage('vueuse-local-token', '')
  // 记住密码
  const rememberMe: RemovableRef<boolean> = useStorage('vueuse-local-rememberMe', false)

  //折叠面板
  const collapse: RemovableRef<boolean> = useStorage('vueuse-local-collapse', config.collapse)

  //背景颜色
  const menuBackground: RemovableRef<string> = useStorage('vueuse-local-menuBackground', config.menuBackground)

  //面包屑
  const isBreadcrumb: RemovableRef<boolean> = useStorage('vueuse-local-isBreadcrumb', config.isBreadcrumb)

  const lockInfo: RemovableRef<Nullable<LockInfo>> = useStorage('vueuse-local-lockInfo', null, undefined, {
    serializer: StorageSerializers.object
  })

  const iconNames: RemovableRef<string[]> = useStorage('vueuse-local-iconNames', [])
  return {
    language,
    token,
    rememberMe,
