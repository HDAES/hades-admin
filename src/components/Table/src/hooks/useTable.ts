...
export function registerTable<T>(tableProps: Props): void {
  ...
  watchEffect(() => {
    loading.value = true
    refreshState.value
    tableProps?.apiList({ page: pagination.currentPage, size: pagination.pageSize }).then(res => {
...
export function useColums(tableProps: Props) {
  const columnsStorage = useStorage<string[]>('vueuse-local-' + tableProps.apiList.name, [])

  const tempColumns: ColumnProps[] = []
  const columnsShowKey: string[] = []

  const isBool = columnsStorage.value.length == 0
