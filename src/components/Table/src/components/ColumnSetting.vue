  ...
  <el-popover placement="bottom-start" :width="250" trigger="click">
    ...
    <div class="column-setting">
      <div style="line-height: 40px">
        <el-checkbox v-model="tableConfig.index">{{ $t('table.config.sequenceCol') }}</el-checkbox>
        <el-checkbox v-model="tableConfig.selection">{{ $t('table.config.selectionCol') }}</el-checkbox>
      </div>
      <draggable v-model="columns" item-key="id" @change="handleDraggable">
        <template #item="{ element }">
          <div class="item" style="line-height: 25px">
            ...
            <el-checkbox v-model="element.show" @change="updateColumns">{{ element.label }}</el-checkbox>
          </div>
        </template>
      </draggable>

      <el-button :icon="RefreshRight" @click="resetColumns">{{ $t('table.config.resetTable') }}</el-button>
    </div>
  </el-popover>
</template>
