  ...
  <template v-if="getShow">
    ...
    <el-form class="enter-x xl:w-90 mt-2" :model="formData" :rules="getFormRules" ref="formRef">
      <el-form-item class="enter-x" prop="account">
        <el-input v-model="formData.account" :placeholder="$t('sys.login.placeholderAccount')" />
      </el-form-item>
      <el-form-item class="enter-x" prop="mobile">
        <el-input v-model="formData.mobile" :placeholder="$t('sys.login.placeholderMobile')" />
      </el-form-item>
      <el-form-item class="enter-x" prop="code">
        <div class="flex flex-1 justify-between">
          <el-input v-model="formData.code" :placeholder="$t('sys.login.placeholderCode')" />
          <CountdownBtn />
        </div>
      </el-form-item>
      ...
      <el-form-item class="enter-x">
        <el-button type="primary" class="w-full" @click="handleLogin">{{ $t('sys.login.signUpFormTitle') }}</el-button>
      </el-form-item>
      <el-form-item class="enter-x">
        <el-button class="w-full" @click="handleBack">{{ $t('sys.login.backSignIn') }}</el-button>
      </el-form-item>
    </el-form>
  </template>
</template>
